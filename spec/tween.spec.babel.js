import Tween from '../src/tween.babel.js';
import { tweener } from '../src/tweener.babel.js';
import { tweenDefaults } from '../src/tween-defaults.babel.js';
...
describe('tween ->', function () {
  describe('extension ->', function() {
    it('should extend `ClassProto`', function () {
      var tween = Tween();
      expect(ClassProto.__mojsClass.isPrototypeOf(tween)).toBe(true);
    });
  ...
  describe('`update` function ->', function() {
    ...
    it('should pass current `progress` to `onUpdate` with `backwardEasingeasing`', function () {
      var ep = -1;
      var p = -1;
      var backwardEasing = mojsEasing.sin.inout;
    ...
    it('should pass current `progress` to `onUpdate` with `easing` #reverse', function () {
      var ep = -1;
      var p = -1;
      var backwardEasing = mojsEasing.sin.in;
    ...
    it('should pass current `progress` to `onUpdate` with `easing` #reverse', function () {
      var ep = -1;
      var p = -1;
      var easing = mojsEasing.sin.inout;
  ...
  describe('`_extendDefaults` function ->', function() {
    it('should call the super', function() {
      spyOn(ClassProto.__mojsClass, '_extendDefaults').and.callThrough();
      var tween = new Tween();
    ...
    it('should parse `easing`', function() {
      ...
      expect(tween._props.easing).toBe(mojsEasing.sin.in);
    });
    ...
    it('should parse `backwardEasing`', function() {
      ...
      expect(tween._props.backwardEasing).toBe(mojsEasing.sin.in);
    });
  ...
  describe('`stagger` properties ->', function() {
    it('parse stagger properties', function () {
      ...
      const delayStaggerFun = (i) => {
        return i * 10;
      };
      delayStaggerFun.__mojs__isStaggerFunction = true;

      const durationStaggerFun = (i) => {
        return i * 50;
      };
      durationStaggerFun.__mojs__isStaggerFunction = true;

      const reverseStaggerMap = [1, 2, 4, 5]
      reverseStaggerMap.__mojs__isStaggerMap = true;

      var tween = Tween({
        index: 3,
        delay: delayStaggerFun,
        duration: durationStaggerFun,
        isReverse: reverseStaggerMap,
        onUpdate: onUpdate
      });

      expect(tween._props.delay).toBe(30);
      expect(tween._props.duration).toBe(150);
      expect(tween._props.isReverse).toBe(5);
